package utilities;

public class IntelliJShortcuts {

//    ctrl + shift + /  - - - >  (/**/)
//    alt  +   /        - - - >  (copies last word)
//    ctrl + space      - - - >  (shows you possible auto-fills)
//    ctrl + arrow key  - - - >  (surf thru sentences one word at a time)
//   shift + shift      - - - >  (pop up window with recent Java classes)
//    alt  + arrow key  - - - >  (flip thru IntelliJ pages)
//    ctrl + o (owl)    - - - >  (insert override method shortcut)
//    ctrl + m          - - - >  (moves you to where your cursor is sitting on the page)
//    ctrl + (>) key    - - - >  (collapses a segment of code)
//
//
//
//
//
//
//
//
//
//
//   select classes + right clck + diagram ---> (creates a diagram to show relationships amongst inherited classes)
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//

}
